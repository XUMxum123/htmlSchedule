...
g_SchedulerInfoCount=1;
g_allSchedulerInfo[0].NAME="SalesSchedule2222222224444444355555555555556";
...
function schedulerDrawEventListUI(target, evtCount, rowIdx,beginIndex) 
{
	...
	if(rowIdx_tmp ==g_expandItemIndex)
	{
		...
		+'<ul><li class="detailfont" style="list-style-type:none;float:left;margin-left:20px;font-weight:bold;width:300px">'+g_allSchedulerInfo[rowIdx].NAME+'</li>'
...
{  
	  ...
	  var tmp=document.getElementById('left_schedule_name00').scrollLeft;
	  
	  document.getElementById('left_schedule_name00').scrollLeft++;
	  console.log("test",document.getElementById('left_schedule_name00').scrollLeft);
	  if(tmp==document.getElementById('left_schedule_name00').scrollLeft)
	  {
		document.getElementById('left_schedule_name00').scrollLeft=0;
	  }
...
/*
	...
	console.log("scrollLeft",obj.scrollLeft);

	
	if (obj.scrollLeft >= 200) 
	{  
		obj.scrollLeft = 0;  
	}  
*/
}  
function stopScollFont(obj)
{  
...
{  
	...
	g_scrollObj=obj;
	g_scollFontTimer = setInterval("_scroll(g_scrollObj)", 20);  
}  
...
function focusScheduleItem(item,bFocus,beginIndex)
{
	...
	else
	{
		...
		if(left_schedule_name)
		{
			if(g_allSchedulerInfo[item+beginIndex].NAME.length>20)
			{
				stopScollFont(left_schedule_name);
			}
